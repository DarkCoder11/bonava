...
const UserStyles = ({ styles, activePackage, activeRequisition }) => {
    ...
    <Col xs={12} xl={9} className="packages">
      <Row className="pack-top">
        <Col
          ...
          style={{
            alignItems:
              (activeStyle && activeStyle.thumbnails.length) || gridKey === 2
                ? "flex-start"
                : "center"
          }}
        >
          {gridKey === 2 ? (
            <Forgeviewer />
          ) : activeStyle && activeStyle.thumbnails.length ? (
            <Carousel
              // slide={false}
              fade
              interval={0}
...
export default connect(mapStateToProps)(UserStyles);
